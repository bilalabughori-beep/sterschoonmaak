    ...
    <main className="flex min-h-screen items-center bg-background px-5 py-10 sm:px-8 sm:py-16">
      <div className="mx-auto w-full max-w-md">
        ...
        <div className="mt-8 rounded-2xl border border-border bg-surface p-7 shadow-sm sm:p-9">
          ...
          {!forgot ? <button type="button" disabled={busy} onClick={() => { setBusy(true); setError(null); void signInWithGoogle().catch((caught) => setError(friendlyAuthError(caught, t("requestFailed")))).finally(() => setBusy(false)); }} className="mt-8 min-h-12 w-full rounded-xl border border-border bg-surface px-5 py-3 text-base font-bold text-navy hover:border-primary disabled:opacity-50">{t("continueGoogle")}</button> : null}
          {!forgot ? <div className="mt-5 flex items-center gap-3 text-xs uppercase tracking-[0.14em] text-muted"><span className="h-px flex-1 bg-border" />{t("or")}<span className="h-px flex-1 bg-border" /></div> : null}
          <form onSubmit={submit} className={`${forgot ? "mt-8" : "mt-5"} space-y-5`}>
