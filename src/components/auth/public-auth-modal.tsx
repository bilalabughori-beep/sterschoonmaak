...
function PublicAuthModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(resetDialogScroll);
    const delayedReset = window.setTimeout(resetDialogScroll, 150);
    ...
    return () => { window.cancelAnimationFrame(frame); window.clearTimeout(delayedReset); document.body.style.overflow = previousOverflow; document.removeEventListener("keydown", onKeyDown); previousFocus.current?.focus({ preventScroll: true }); };
  }, [onClose]);

  const changeMode = (next: AuthMode) => { setMode(next); setError(null); setMessage(null); setPassword(""); setConfirmation(""); resetDialogScroll(); window.requestAnimationFrame(resetDialogScroll); };
