...
function PublicAuthModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const frame = window.requestAnimationFrame(resetOverlayScroll);
  ...
  const changeMode = (next: AuthMode) => { setMode(next); setError(null); setMessage(null); setPassword(""); setConfirmation(""); if (overlayRef.current) overlayRef.current.scrollTop = 0; window.requestAnimationFrame(() => { if (overlayRef.current) overlayRef.current.scrollTop = 0; }); };
