...
export type ComplaintReply = { id: string; complaint_id?: string; author_user_id?: string; author_role?: "client_admin" | "site_owner"; body: string; email_status: "pending" | "sent" | "failed"; email_attempts?: number; email_sent_at?: string | null; email_last_error?: string | null; resend_message_id?: string | null; created_at: string };
export type Complaint = { id: string; reference: string; created_at: string; locale: string; customer_name: string; customer_email: string; customer_phone?: string | null; subject: string; message: string; source_path?: string | null; status: "new" | "in_progress" | "resolved" | "closed"; internal_notes?: string | null; email_status: "pending" | "sent" | "failed"; email_sent_at?: string | null; email_attempts: number; email_last_error?: string | null; replies?: ComplaintReply[] };
export type CustomerComplaint = { id: string; reference: string; created_at: string; subject: string; message: string; status: "new" | "in_progress" | "resolved" | "closed"; locale: string; email_status?: "pending" | "sent" | "failed"; replies?: Pick<ComplaintReply, "id" | "body" | "email_status" | "created_at">[] };
...
export async function fetchCustomerComplaints() { return (await call("/account/complaints") as { items: CustomerComplaint[] }).items; }
export async function fetchCustomerComplaint(id: string) { return await call(`/account/complaints/${encodeURIComponent(id)}`) as CustomerComplaint; }
export async function createCustomerComplaint(payload: Record<string, string>, locale: string, sourcePath: string) { return await call(`/account/complaints?locale=${encodeURIComponent(locale)}&sourcePath=${encodeURIComponent(sourcePath)}`, { method: "POST", body: JSON.stringify(payload) }) as { ok: true; reference: string; createdAt: string; status: string; emailStatus: "sent" | "failed" }; }
...
export async function createComplaintReply(id: string, body: string) { return await call(`/admin/complaints/${encodeURIComponent(id)}/replies`, { method: "POST", body: JSON.stringify({ body }) }) as ComplaintReply; }
export async function retryComplaintReply(complaintId: string, replyId: string) { return await call(`/admin/complaints/${encodeURIComponent(complaintId)}/replies/${encodeURIComponent(replyId)}/retry`, { method: "POST", body: "{}" }) as ComplaintReply; }
