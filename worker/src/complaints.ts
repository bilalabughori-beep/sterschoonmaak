...
export async function sendComplaintEmail(env: Env, row: Record<string, unknown>, updatedBy?: string) {
  ...
  const text = isEnglish
    ...
    : `Beste ${String(row.customer_name ?? "")},\n\nWe hebben uw klacht ontvangen en geregistreerd.\n\nReferentie: ${String(row.reference)}\nOnderwerp: ${String(row.subject ?? "")}\n\nWe bekijken uw melding zo snel mogelijk. Bewaar uw referentienummer indien u later contact met ons opneemt.\n\nMet vriendelijke groet,\nSter Schoonmaak\ninfo@sterschoonmaak.be`;
  const response = await fetch("https://api.resend.com/emails", { method: "POST", headers: { authorization: `Bearer ${apiKey}`, "content-type": "application/json" }, body: JSON.stringify({ from: env.RESEND_FROM_EMAIL?.trim() || "Ster Schoonmaak <info@sterschoonmaak.be>", to: [destination], reply_to: "info@sterschoonmaak.be", subject, html, text }) });
